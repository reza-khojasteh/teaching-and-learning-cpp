...
int main() {
	// A polymorphic object that can change its dynamic type throughout its lifetime.
	...
	ptr = new Employee(1, "Jane", "Doe");
	display(*ptr);
	delete ptr;

	// Later on, this pointer can point to the derived class object:
	ptr = new HourlyBasedEmployee(2, "John", "Doe", 43.5, 50.0);
	display(*ptr);
	delete ptr;
